...
public abstract class WebSocketServer extends org.java_websocket.server.WebSocketServer {
    ...
    protected final Table<String, String, Set<WebSocketServerReceiver>> WEB_SOCKET_SERVER_RECEIVER_LIST_TABLE = new Table<>();
    // websocket 发送者table，通过loop,subject确定发送器，只有绑定过WebConnection才会被添加，即使这个连接已经中断
    protected final Table<String, String, Set<WebSocketServerSender>> WEB_SOCKET_SERVER_SENDER_LIST_TABLE = new Table<>();
    // 注册的所有的websocket Server operator，可能是绑定完成的，也可能是没有完成的
    protected final List<WebSocketServerOperator> WEB_SOCKET_SERVER_OPERATOR_LIST = new ArrayList<>();
    ...
    public boolean webSocketConnectionActive(WebSocketConnection webSocketConnection) {
        return (!webSocketConnection.getWebSocket().isClosed()) && (WEB_SOCKET_CONNECTION_LIST.contains(webSocketConnection));
    }
    ...
    public static class WebSocketServerBuilder {
         ...
         * @return 返回值
         */
        private WebSocketConnection createWebSocketConnection(WebSocket conn, ClientHandshake handshake) {
            try {
                //将对应的变量修改为可访问
         ...
         * @return 绑定的接收器和发送器
         */
        private Map<String, List<? extends WebSocketServerOperator>> getAllWebSocketOperators(WebSocketConnection webSocketConnection) {
            ...
            //遍历所有的
            for (WebSocketServerOperator webSocketServerOperator : webSocketConnection.getWebSocketServerOperatorList()) {
                // 如果是接收器
        ...
        public WebSocketServer build() {
            return new WebSocketServer() {
                ...
                public void onOpen(WebSocket conn, ClientHandshake handshake) {
                    ...
                    //创建对应的连接对象
                    WebSocketConnection webSocketConnection = createWebSocketConnection(conn, handshake);
                    //连接列表中增加该连接
                    ...
                    for (WebSocketServerOperator webSocketServerOperator : WEB_SOCKET_SERVER_OPERATOR_LIST) {
                        webSocketServerOperator.bind(webSocketConnection);
                    }
                    // 遍历所有的连接，找到所有已绑定的WebSocketOperator
                    for (WebSocketConnection perWebSocketConnection : WEB_SOCKET_CONNECTION_LIST) {
                        //获得活跃连接上绑定的所有操作器
                        Map<String, List<? extends WebSocketServerOperator>> map = getAllWebSocketOperators(perWebSocketConnection);
                        List<WebSocketServerReceiver> webSocketServerReceivers = (List<WebSocketServerReceiver>) map.get("webSocketReceivers");
                        List<WebSocketServerSender> webSocketServerSenders = (List<WebSocketServerSender>) map.get("webSocketSenders");
                        //遍历所有接收器
                        for (WebSocketServerReceiver webSocketServerReceiver : webSocketServerReceivers) {
                            //解析注释
                            Map<String, String> webSocketControllerMap = WebSocketUtil.parseWebSocketController(webSocketServerReceiver);
                            ...
                            if (!StringUtils.isAnyEmpty(loop, subject)) {
                                //添加到对应的表格中去
                                Set<WebSocketServerReceiver> webSocketServerReceiverSet = WEB_SOCKET_SERVER_RECEIVER_LIST_TABLE.get(loop, subject);
                                if (null == webSocketServerReceiverSet) {
                                    webSocketServerReceiverSet = new HashSet<>();
                                    WEB_SOCKET_SERVER_RECEIVER_LIST_TABLE.put(loop, subject, webSocketServerReceiverSet);
                                }
                                webSocketServerReceiverSet.add(webSocketServerReceiver);
                            }
                        }
                        //遍历所有发送器
                        for (WebSocketServerSender webSocketServerSender : webSocketServerSenders) {
                            //解析注释
                            Map<String, String> webSocketControllerMap = WebSocketUtil.parseWebSocketController(webSocketServerSender);
                            String loop = webSocketControllerMap.get("loop");
                            String subject = webSocketControllerMap.get("subject");
                            //两个值都不为空，则将其加入到对应的操作列中
                            if (!StringUtils.isAnyEmpty(loop, subject)) {
                                //添加到对应的表格中去
                                Set<WebSocketServerSender> webSocketServerSenderSet = WEB_SOCKET_SERVER_SENDER_LIST_TABLE.get(loop, subject);
                                if (null == webSocketServerSenderSet) {
                                    webSocketServerSenderSet = new HashSet<>();
                                    WEB_SOCKET_SERVER_SENDER_LIST_TABLE.put(loop, subject, webSocketServerSenderSet);
                                }
                ...
                public void onClose(WebSocket conn, int code, String reason, boolean remote) {
                    try {
                        ...
                        //找到对应的活跃连接
                        WebSocketConnection activeWebSocketConnection = getActiveWebSocketConnection(conn);
                        if (ObjectUtils.isNotEmpty(activeWebSocketConnection)) {
                            //设置关闭时间
                            activeWebSocketConnection.setCloseTime(System.currentTimeMillis());
                            //调用关闭监听器
                ...
                public void onMessage(WebSocket conn, String message) {
                    try {
                        ...
                        if (ObjectUtils.isNotEmpty(activeWebSocketConnection)) {
                            //设置更新时间
                            activeWebSocketConnection.setUpdateTime(System.currentTimeMillis());
                            //调用接收信息监听器
                            activeWebSocketConnection.getMessageListener().accept(message);
                            ...
                            Set<WebSocketServerReceiver> webSocketServerReceivers =
                                    WEB_SOCKET_SERVER_RECEIVER_LIST_TABLE.get(webSocketPackage.getLoop(), webSocketPackage.getSubject());
                            //遍历接收器
                            for (WebSocketServerReceiver webSocketServerReceiver : webSocketServerReceivers) {
                                //调用回调函数
                                webSocketServerReceiver.receive(activeWebSocketConnection, webSocketPackage);
                            }
                        }
                    } catch (Exception e) {
                        throw SkyeUtilsExceptionFactory.createException(
                                SkyeUtilsExceptionType.WebSocketServerMessageEventException);
                    }
                ...
                public void onWebsocketPong(WebSocket conn, Framedata f) {
                    ...
                    //更新对应的连接的更新时间
                    getActiveWebSocketConnection(conn).setUpdateTime(System.currentTimeMillis());
                }
            };
        }
    }
