...
/**
 * @Description 可逆的加解密算法 考虑到可能在整个项目中存在使用多个加密方式的情况下
 * @Author Skye
...
public class JwtUtil {
    ...
    public static String encode(EncodeOrigin encodeOrigin, Map<String, Object> params) {
        ...
        JWTCreator.Builder builder = JWT.create()
                ...
                // 设置有效期，一天有效期
                .withExpiresAt(new Date(encodeOrigin.getIssueTime().getTime() + encodeOrigin.getExpireTime()));
        //添加对应的自定义参数
    ...
    public static DecodeInfo decode(String encodeStr, DecodeOrigin decodeOrigin, Map<String, Class<?>> paramClasses) {
        ...
        // 根据指定算法，构建一个验证器
        JWTVerifier verifier = JWT.require(decodeOrigin.getAlg())
                // 指定发行者
        ...
        for (Entry<String, Class<?>> entry : paramClasses.entrySet()) {
            ...
            Object value = SerializeUtil.convertJsonToBeanByClass(originStr.getClaim(key).asString(), paramClass);
